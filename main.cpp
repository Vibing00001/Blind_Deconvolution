...
struct output buildPyramid(struct input &data, struct output &answer)
{
    ...
    while (mkpnext > smallestScale && nkpnext > smallestScale && lamnext * data.lambdaMultiplier < data.largestLambda)
    {
        scales = scales + 1;
...
void helper(cv::Mat image)
{
    ...
    // params.niters = 1000;
    params.niters = 300; // changed to make faster.
...
int main(int argc, char *argv[])
{
    ...
    // Added code to record how long the algorithm will take to finish
    ...
    cout << "Operation completed in " << duration << " seconds" << endl;
